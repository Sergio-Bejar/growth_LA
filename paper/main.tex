...
Loayza2007, HnatkovskaLoayza2005}. Yet developing countries with similar
macroeconomic fundamentals experience markedly different levels of
...
macroeconomic fundamentals and regime type. Second, it shows that the effect
...
behavior. This argument is intuitive and supported by growing evidence, but
it captures only one of the two channels through which polarization affects
...
leading to short-termist and clientelistic governance. A centrist party
...
regardless of their left-right position.
...
composition of government is ideologically ambiguous. Our argument implies
that the party system is an upstream determinant of the EPU: higher
programmatic differentiation at the party system level reduces the
residual uncertainty that the EPU captures.
...
nationalist or redistributive agenda.
...
experiment or credible instrument. The evidence is consistent with the
causal interpretation but does not definitively establish it.
...
Figure~\ref{fig:period} presents split-sample results and the country-level
decomposition of changes in growth volatility across periods.
...
term is significant. Panel D reveals why: the post-2007 period scrambles the
regional pattern in ways orthogonal to domestic party systems. Uruguay and
...
Figure~\ref{fig:lags} presents the lag structure systematically across the
Latin American and global samples.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_mechanism.png}
...
mediator produces zero attenuation of the polarization coefficients. The
...
\citep{KokashviliBarbakadze2020}, while the growth-stabilizing effect
operates through domestic channels that respond to total policy uncertainty.
At the firm level, \citet{Zhu2024} shows that a one-standard-deviation
increase in political polarization reduces firm investment by 1\% in the
United States, confirming that polarization affects real investment
decisions, though the effect in that context is linear rather than
U-shaped.
...
countries, would allow a more targeted test. Until such data are available,
the null mediation results across all nine channels narrow the theoretical
...
Figure~\ref{fig:global} presents results from the global extension.
...
We extend the analysis to a global sample using V-DEM's $v2cacamps$
...
non-Latin American democracies (Panel B) recovers the U-shaped pattern
...
replicates globally (Panel E), consistent with the scope condition.
...
competition is itself a development variable. This reframes a set of
institutional design questions. The conventional advice to developing
